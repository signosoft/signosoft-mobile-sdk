...
**A `--dart-define` is compiled into the build.** The token is baked into the
app, not read at launch, so relaunching from the home screen reuses whatever
token that build carries — and once it has been signed, the ceremony reopens the
*already-signed* document and closing it reports `Cancelled`. That is the SDK
behaving correctly ([why](../../docs/INTEGRATION.md#5-the-four-outcomes)); it
just is not a fresh run. Every attempt needs its own `flutter run` with a newly
minted token.
...
iPhone as well as iPad — and so does an Android emulator:
...
`flutter devices` lists a booted emulator as `emulator-5554` or similar. The
demo's Android application id is `com.signosoft.medicly_demo`. Boot the emulator
detached first — if it runs in the foreground, freeing your shell kills the
device:

```bash
<android-sdk>/emulator/emulator -avd <name> -no-snapshot-save -gpu host >/dev/null 2>&1 &
```

```bash
flutter run -d emulator-5554 --dart-define=BIOID=<your bioid>
```

**Expect the first ceremony to be slow, and possibly to time out.** On the
emulator the shell takes 15–20 s to become interactive, and a first open can
exceed the SDK's 45 s watchdog and end in `loadTimeout`. This is the emulator's
network stack: each of the 35–50 requests the shell makes costs about 1 s there
against about 50 ms from the Mac. If you hit it, just open the ceremony again —
the token is untouched, because only a terminal outcome spends one.

`-gpu host` is worth passing anyway for a smoother UI, but it does **not** fix
the load time: a cold run with `-gpu host` timed out exactly the same way.
Measurements are in
[INTEGRATION.md](../../docs/INTEGRATION.md#known-limitations).

Gradle needs **JDK 17** — AGP 9 rejects a newer one, and the failure reads as a
build-script error rather than a JDK mismatch. `flutter run` uses the JDK from
`flutter config --jdk-dir`, so it needs no environment; invoking `./gradlew`
directly does:

```bash
JAVA_HOME=/path/to/jdk-17 ANDROID_HOME=/path/to/android-sdk ./gradlew test
```

If a token you know is good reports `sessionFailed` / `DOC_LINK_NOT_EXIST`,
stale WebView storage from an earlier install is the first thing to rule out —
`adb uninstall com.signosoft.medicly_demo`, then run again. Reinstalling over the
top does not clear it.
...
[INTEGRATION.md §2](../../docs/INTEGRATION.md#2-host-permissions).

`android/app/src/main/AndroidManifest.xml` is the Android half: the `CAMERA` and
`RECORD_AUDIO` `<uses-permission>` declarations, and a
`res/xml/network_security_config.xml` allowing cleartext to `localhost` and
`10.0.2.2` for a locally served `BASE_URL`. Same rule, same reason — Android
attributes permissions to the host app, and a web origin cannot be granted a
capability the app itself does not hold. Delete the network security config for a
production build.
